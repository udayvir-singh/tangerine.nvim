...
local function validate_list(key, list, scm)
  _G.assert((nil ~= scm), "Missing argument scm on fnl/tangerine/utils/env.fnl:204")
  _G.assert((nil ~= list), "Missing argument list on fnl/tangerine/utils/env.fnl:204")
  _G.assert((nil ~= key), "Missing argument key on fnl/tangerine/utils/env.fnl:204")
...
local function validate(tbl, schema0)
  _G.assert((nil ~= schema0), "Missing argument schema on fnl/tangerine/utils/env.fnl:215")
  _G.assert((nil ~= tbl), "Missing argument tbl on fnl/tangerine/utils/env.fnl:215")
  for key, val in pairs(tbl) do
    ...
    local _12_ = {get_type(scm), scm[1]}
    if ((_G.type(_12_) == "table") and ((_12_)[1] == "string") and ((_12_)[2] == nil)) then
      validate_type(key, val, scm)
    elseif ((_G.type(_12_) == "table") and ((_12_)[1] == "table") and ((_12_)[2] == nil)) then
      validate(val, scm)
    elseif ((_G.type(_12_) == "table") and ((_12_)[1] == "list") and ((_12_)[2] == "oneof")) then
      validate_oneof(key, val, scm[2])
    elseif ((_G.type(_12_) == "table") and ((_12_)[1] == "list") and ((_12_)[2] == "array")) then
      validate_array(key, val, scm[2])
    elseif ((_G.type(_12_) == "table") and ((_12_)[1] == "list") and true) then
      local _ = (_12_)[2]
      validate_list(key, val, scm[1])
    else
    end
...
local function pre_process(tbl, schema0)
  _G.assert((nil ~= schema0), "Missing argument schema on fnl/tangerine/utils/env.fnl:229")
  _G.assert((nil ~= tbl), "Missing argument tbl on fnl/tangerine/utils/env.fnl:229")
  for key, val in pairs(tbl) do
...
local function rget(tbl, args)
  _G.assert((nil ~= args), "Missing argument args on fnl/tangerine/utils/env.fnl:242")
...
local function env_get_conf(opts, args)
  _G.assert((nil ~= args), "Missing argument args on fnl/tangerine/utils/env.fnl:254")
