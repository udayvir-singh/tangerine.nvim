...
(lambda eval.string [str ?opts]
  ...
  (let [fennel   (fennel.load)
    ...
    (local (ok result)
      (xpcall #[(fennel.eval str {: filename :compilerEnv _G :useBitLib true})]
              #(err.handle $1 opts)))
    (if (not ok)
        (lua "return false"))
    (dp.show result opts)
...
(lambda eval.peek [start end ?opts]
  ...
  (let [fennel  (fennel.load)
    ...
    (err.clear) ;; clear previous errors
    (local (ok result)
      (xpcall #(fennel.compileString lines {:filename (or opts.filename bufname) :compilerEnv _G :useBitLib true})
